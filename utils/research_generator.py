...
class ResearchGenerator:
    def __init__(self, topic, keywords, research_questions, api_keys: Dict[str, str],
                 system_prompt, deep_research_enabled: bool = False, model_name: str = 'gemini-1.5-flash', max_retries=3, timeout=60, spinner_update_callback=None):
        """
    ...
    def _initialize_api_clients(self):
        ...
        # Get the first part of the model name (e.g., 'gemini' from 'gemini-1.5-flash')
        model_prefix = self.model_name.split('-')[0]  # Take the first element of the split list

        api_key = self.api_keys.get(model_prefix)

        if not api_key:
            if self.spinner_update_callback:
                self.spinner_update_callback(f"Warning: API key not provided for model prefix: {model_prefix}. {self.model_name} will not be available.")
        ...
        if model_prefix == 'gemini':
            try:
                ...
                if self.spinner_update_callback:
                    self.spinner_update_callback(f"Error: Failed to configure Gemini API: {e}")
            ...
            try:
                ...
                if self.spinner_update_callback:
                    self.spinner_update_callback(f"Error: Failed to configure OpenAI API: {e}")
            ...
            try:
                ...
                if self.spinner_update_callback:
                    self.spinner_update_callback(f"Error: Failed to configure Anthropic API: {e}")
                logging.error(f"Failed to configure Anthropic API: {e}")
        else:
            if self.spinner_update_callback:
                self.spinner_update_callback(f"Warning: No API client configured for model: {self.model_name}")
    ...
    def _make_api_call_with_retry(self, prompt, section_name):
        ...
        for attempt in range(self.max_retries):
            try:
                if self.spinner_update_callback:
                    self.spinner_update_callback(f"Crafting the {section_name} with {self.model_name} (attempt {attempt + 1}/{self.max_retries})..")
    ...
    def generate_section(self, section_data: Any, spinner_update_callback=None):
        """
        ...
        try:
            ...
            if isinstance(section_data, dict):
                ...
                section_prompt = f"Create a {section_name} for a research report on the topic: {{topic}}. Keywords: {{keywords}}. Research questions: {{research_questions}}"
            else:
                raise ValueError("Invalid section_data type. Must be string or dictionary.")
            
            # Replace placeholders in the section prompt
            formatted_section_prompt = section_prompt.format(
                topic=self.topic,
            ...
            final_prompt = (
                ...
                f"{formatted_section_prompt}"
            )
            
            if word_count:
                final_prompt += f" Ensure the content is approximately {word_count} words. "
                if self.deep_research_enabled:
                    final_prompt += "Provide a highly detailed and comprehensive response."
    ...
    def generate_summary(self, full_report_content: str) -> str:
        ...
        try:
            ...
            prompt = (
                f"{self.system_prompt}\n\n"
                f"Please provide a {summary_detail_instruction} executive summary (around {summary_word_count} words) of the following research report. "
                ...
                f"If deep research is enabled, provide a more extensive and in-depth summary.\n\n"
                f"Research Report:\n{full_report_content}"
            )
